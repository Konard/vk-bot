...
const messages = [
  "Читал ли ты «Гарри Поттер и Методы Рационального Мышления»?",
  "Знаком ли ты с HPMOR?",
  "Читал ли ты рациональную версию Гарри Поттера?",
  "Слышал ли ты о «Методах Рационального Мышления»?",
  "Что думаешь о HPMOR — «Гарри Поттер и Методы Рационального Мышления»?",
  "Читал ли ты фанфик «Гарри Поттер и Методы Рационального Мышления»?",
  "Знаком ли ты с работой Элиезера Юдковского — HPMOR?",
  "Что думаешь о рациональной версии Гарри Поттера?",
  "Читал ли ты «Harry Potter and the Methods of Rationality»?",
  "Интересует ли тебя рациональное мышление в контексте Гарри Поттера?"
];

async function askAboutHPMOR(context) {
  ...
  for (let offset = 0; offset < limit; offset += step) {
    ...
    for (const friend of response.items) {
      ...
      const minimumInterval = 14; // 2 weeks interval for HPMOR topic
      ...
      console.log(`HPMOR question for friend ${friend.id} is added to queue.`);
      await sleep((30 * second) / ms);

      friendsProcessed++;
      console.log('friendsProcessed:', friendsProcessed);
      if (friendsProcessed >= maxGreetings) {
        console.log(`No more friends to greet with HPMOR topic, ${maxGreetings} limit reached.`);
        return;
      }
    }
  }
...
const trigger = {
  ...
  action: async (context) => {
    return await askAboutHPMOR(context);
  }
};
